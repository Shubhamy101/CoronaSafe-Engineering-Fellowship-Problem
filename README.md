# CoronaSafe Engineering Fellowship Test Problem (C++)
...
I have done the problem using Language C++ on Windows Platform. On the Command Line Interface and IDE while running program, I was able to execute all tests cases and got satsifactory result for all test cases. But using ```npm test```, I was getting only 12/16 test cases passed. Among 4 failed cases, the 3 cases which were failed were related to string mismatch as it was showning a . (dot) for every '\n' or endl used in line. I tried but couldn't figure out a way to remove it. Also , I thought the reason i am getting this error is because i am not using a Linux Environment. The other error was the ```report completed and pending todos```. I was getting accurate result when running on a IDE, but got this error on ```npm test``` for the same input. So, please make sure to check the code execution once without using ```npm test``` on any C++ IDE or Platform. The IDE which I used was CS50 IDE which is a cloud-based Ubuntu environment. I was able to pass all test cases for same input when executing on the IDE but facing issues when using the ```npm test``` on Windows PowerShell and CMD.
